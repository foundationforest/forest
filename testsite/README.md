...
A static site with one test page. No server, no framework: everything runs in the page. Nothing here is a product, and nothing here is deployed.
...
The page uses its own origin as the passkey's relying party. In Forest the relying party is `forest.foundation`.
...
3. Deploy. Vercel serves the site over HTTPS at its own address. The page is at `/keys/`.
...
Any other static host with HTTPS works the same way: serve `testsite/dist` as it is. The page needs a trailing slash (`/keys/`), because it loads its scripts by relative path. `vercel.json` redirects `/keys` to `/keys/`.
