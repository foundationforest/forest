...
test('the directory: each linked market file, and nothing else', async () => {
  ...
  assert.equal(d.postMarket('plumbing'), 'plumbing')
  assert.equal(d.postMarket('plumber'), null, 'no aliases: another spelling is another name, outside the directory')
  assert.equal(d.badgeScope('plumber/seller'), null)
