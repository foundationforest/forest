...
| Names | v1 has only random handles, always free: the host gives one at creation under the app's domain, and every badged profile gets one under forest.foundation, like `k7m2q.forest.foundation`, which is its profile page and AT Protocol handle. Chosen names are v2, sold by auction once there are enough people to make prices real | Ours, small | Foundation |
...
- **Registration.** One transaction carries: the market name, the profile's DID, one proof whose scope is the market name, and the fee in an accepted token. One rule: 25 cents in USDC, always; other accepted tokens at the fee set for them. There are no free slots, no numbered codes and no vouchers in the program.
...
- Deposit address. Each escrow has its own token account. Money arrives by plain transfer from anywhere: the app, a pay link, a friend, an AI. The escrow counts as funded when the balance is at least the amount; anything above the amount goes back to the buyer at the end. The buyer's refund address is the buyer's standard token account for the escrow's token, fixed at creation by the buyer's key and the token; the instructions anyone may send pay the buyer only there.
- Service time, optional. The clock starts at the latest of the service time, the funding and the seller's acceptance. Funding always counts, so an old invoice paid late cannot be released at once.
- Acceptance. The seller accepts the escrow as it stands before anything but paying in full can happen. Before acceptance only three things can end a funded escrow: the buyer's approval of everything to the seller (paying in full never needs the seller's consent), the buyer's withdrawal of everything, and, once it has timed out, anyone sending everything back to the buyer: after its last cancellation deadline, or 30 days after its funding if it has no steps, with the rent to the rent payer and the receipt kept with the outcome "never accepted", because a sponsor's rent must not depend on the buyer coming back. No objection, no silence, no arbiter, no agreement, no seller's cancellation until the seller accepts; after it, everything runs as below. An escrow the seller opens is accepted from creation: an invoice. The clock never starts before acceptance. Because otherwise a stranger could open an escrow naming any seller, lock it, or name its arbiter and its clock.
...
- Late money. A payment that lands at an ended escrow's deposit address (a second tap on an old pay link is almost always the buyer's own money) goes back to the buyer's refund address. Anyone may send it, and may create the refund account if it does not exist; the receipt does not change. Pay links are one-time: the client builds one only for an escrow still waiting for its money.
- Rent. Whoever pays the creation rent is recorded. The deposit account's rent goes back to them at the end; the receipt keeps its rent-exempt minimum. Anything above that minimum, as Solana cuts the rent rate or if SOL is sent to it, goes back to the rent payer by a sweep anyone may send, which can never take the account below the minimum or touch anything else.
...
- Every state change is emitted in the log with amounts, so indexes can read outcomes: created, accepted, funded, approved, released by silence, objected, agreed, arbitrated, cancelled by buyer, cancelled by seller, withdrawn, never accepted, ended, and closed (never funded); and, outside any ending, late money returned and rent swept. The ending is in the account too.
- Wallets. A person has one central wallet and one wallet per profile. The central wallet is where money enters from a ramp and leaves to one; it never pays a seller and never receives from a buyer. Every deal touches only profile wallets, so receipts bind to profiles. Moves between the central wallet and a profile wallet are where profiles could be linked on chain, going in and, above all, coming out, because every seller cashes out to one place. A privacy pool and a ramp are features an app offers and a user chooses; any app may offer any provider. Hinkal (live on Solana, audited, compliance built in) is the first pool candidate and Crossmint the first ramp. Nothing about them is built into the foundation. An app that moves money through a pool moves round amounts and waits a random while, since equal amounts minutes apart are a link anyone can read; a move the user makes without one is direct, and the button says it connects the two on chain.
...
- An index weighs a deal by who said yes. Both (the buyer paid and the seller accepted or invoiced) counts in full. A one-tap payment with no acceptance is a real payment receipt but one-sided until the seller reviews the same deal id. An escrow nobody accepted and nobody paid in full counts for nothing.
...
- v1 has only random handles: the host gives one at creation (under the app's domain) and the foundation gives one under forest.foundation when a profile is badged, like `k7m2q.forest.foundation`, which is the profile page and the share link. Random names link nothing, and they are always free.
- Chosen names are v2, sold by auction once there are enough people to make prices real. Choosing the same word on two profiles would link them; the user's choice, said in the copy.
...
Money in Soil follows the wallets rule under "Escrow": one central wallet per person and one wallet per profile. Ramp links pay into and out of the central wallet only; every deal is paid from and to profile wallets only. A privacy pool and a ramp are features Soil offers and the user chooses; any app may offer any provider. Hinkal (live on Solana, audited, compliance built in) is the first pool candidate and Crossmint the first ramp. When the user moves money between the central wallet and a profile wallet through the pool, Soil moves round amounts after a random wait; when the user moves it directly, the button says it connects the two on chain. Nothing about either is built into the foundation: no code, no embed.
...
Which three markets. Default silence days per market. Whether one stablecoin holds for EU users. Which ramps accept a prefilled link with no partner account. Whether a moved passkey keeps its PRF secret (the design assumes not). Whether the registry should also emit an attestation other Solana apps can read (parked). Whether Crossmint or Onramper accepts a sole proprietor for production. Whether Crossmint's bank rails cover the first sellers' countries. Whether the Semaphore team publishes a transcript for the later artifacts. The per-market completeness circuit and its ceremony (v1; the ceremony needs real independent contributors, so it waits for a public). Whether scopes are per market or per market and role, decided when the first real markets are written. When a second list is opened. A later circuit proving the profile's key and the identity secret come from one seed, which closes badge selling entirely; v1. When chosen names (v2) open and how their auction runs: its format, whether a name is held for good or renewed, and where the money goes. Whether a badged profile's active handle moves from the host's to the foundation's by default. Which wallet holds a pool's compliance attestation (Hinkal's access token is minted after a check by an exchange, custodian or KYC provider): on the central wallet it links a person to what the ramp already knows; on a profile wallet it would put a person next to a profile. Whether an app offering a pool needs its own lawyer pass, now that the foundation's gate is gone.
