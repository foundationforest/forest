...
- **Accepted tokens.** USDC always (sealed, so registration can never be halted). Others added by the treasury key, restricted to dollar stablecoins pegged one to one. The rule is 25 cents in the mint's own decimals: the program reads a mint's decimals when the mint is accepted and stores them next to it, and a registration pays 0.25 × 10^decimals of that mint. Nothing about decimals is a sealed constant, so a future dollar token with more or fewer decimals than USDC is not locked out.
