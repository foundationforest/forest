# Forest: foundation handoff (September 23, 2026)
...
|---|---|---|---|
| Keys recipe | Passkey secret (WebAuthn PRF) to seed; seed to each profile's control key, signing key, wallet and DID, plus one identity secret and one central wallet per person. Seed file format for extra passkeys. Paper export | Ours: a spec and a small library from existing parts | The user's device |
...
| Registry | Sealed program: lists of verified humans, one code per human per market, 25 cents per registration, the profile's wallet signing | Ours; Semaphore's circuit unchanged, groth16-solana, the Poseidon syscall | Solana |
| Fee payer, registrations | Sponsors registrations in standard markets whose proof checks out, rate-limited; co-signs for fees while the profile signs for consent | Kora, configured, no custom code | Railway |
...
| Issuer flow | Face check result to identity commitment to list insert, in batches | Ours, small | Railway |
| Names | A random handle under forest.foundation for every badged profile | Ours, small | Vercel and Supabase, with the index |
...
- **Issuers** vouch that a person is one human. The foundation runs the first, with Didit's face check. An issuer that inserts into the registry's lists needs the treasury to add its key, up to eight per list; any issuer can sign credentials.
...
- **Apps** are where people act. The first is Soil.
...
| Keys recipe, seed file format, paper export | An instance of the host; a seed-file store |
| Registry, escrow, host software, carrier, index, ranking, registration fee payer, names, issuer | A fee payer for everything else (fund, release, send) |
...
Repos: `foundationforest/forest` (all foundation code, Apache 2.0) and `foundationforest/markets` (market and category files, CC0). The product org holds `soil`. Forest lives at forest.foundation; Soil at soil.host (not bought yet).
...
- **Paper export.** The seed as 24 English words. They open everything; they are for paper only.
- **The passkey's domain.** The passkey belongs to forest.foundation. Products that use it are listed in `https://forest.foundation/.well-known/webauthn` (related origins), Soil first, so one passkey serves every product.
- **The identity secret.** One per person, with no profile index, so the registry holds one entry per human while profiles stay apart. Only its commitment leaves the device, once, to the issuer. Losing the seed loses it: the badges stay, and getting back on the list takes another face check.
...
- **Lists.** Each verified human adds one identity commitment, made on their device, to a list: a Merkle tree of depth 32 that the list's issuer keys can insert into, hashed exactly as Semaphore's circuit hashes (Poseidon, through Solana's syscall). The program holds a set of lists and starts with one. When a second list opens, new joiners are assigned randomly across all open lists, so a list never tells when someone joined. The list is not split on day one. Issuers insert in batches.
...
- **Free registration lives outside the program.** The foundation's sponsor pays the fee under its own policy, which can change without touching the program: only markets in the `markets` repo, only proofs that check out, rate-limited. The sponsor co-signs for fees; the profile signs for consent. When the directory is large, the sponsor can also require a proof of "fewer than N badges across the directory", the same walk as the completeness proof below.
...
- **The dials.** The treasury accepts a token at a fee, opens a list, closes a list to new members for good, adds or removes a list's issuer keys, and hands itself over. Nothing else. Proofs against a closed list stay valid forever, and nothing deletes a list.
...
- **No removals.** Once a human is in a list, they stay. Removing an issuer removes no one. A duplicate that passes the face check cannot be undone in v1.
...
- **Refund address.** The buyer's standard token account for the escrow's token, fixed at creation by the buyer's key and the token. The two returns anyone may send (late money and the unaccepted timeout) pay the buyer only there, making it first if it is missing. Every other ending pays the buyer at any token account the buyer owns.
...
- **A review is a signed claim** by one profile about another, about one deal. Two reviews across one deal id are the two sides' receipts; no other record is needed for that.
...
- **Review:** about whom (a DID), a rating from 1 to 5, text, and an optional `escrow` field holding the escrow's address, which is the deal id. Decided, not built: the pointer becomes a deal id, so a deal with no escrow can be pointed at, and the rating becomes optional, so a review can say only "I dealt with this person on this deal".
...
- v1 has only random handles, always free. The host gives one at creation, under the app's domain (not built). The foundation gives one under forest.foundation when a profile is badged, like `k7m2q.forest.foundation`, which is the profile page and the share link. Random names link nothing.
- Names under forest.foundation are for badged profiles only. The foundation never judges disputes.
- Chosen names are v2, sold by auction once there are enough people to make prices real. Choosing the same word on two profiles links them: the user's choice, said in the copy.
- Any app or host claims names through the same open endpoint; the interface is theirs. Profiles without a foundation handle are still reachable by DID on forest.foundation.
...
**Index (planned).** Reads the carrier, scores with an open algorithm, and serves category pages (published once a category is dense enough), profile pages, the badge, the pay link and a machine endpoint. Reading is public HTTP with no keys: stable URLs, JSON, no session. The read skill, a plain text file teaching any AI where the endpoints are and how to read them, is published at forest.foundation and in `index/`. Writing needs the user's keys, so it goes through the door in Soil. It counts a badge only when the entry's wallet is the profile's declared wallet, weighs deals by who said yes, and decides which tokens it counts. The reading rules adversarial review 1 found (only log entries the programs themselves wrote, market names compared byte for byte, its own archive of the logs) are in `docs/decisions/adversarial-review-1.md`. The pay link is a one-time Solana Pay link naming the escrow's address, from which the wallet finds the deposit account.

**Fee payer (planned).** Kora, configured, with no custom code. For registrations it sponsors only markets in the `markets` repo whose proof checks out, rate-limited. A registration has three signing roles (the payer of network fees and rent, the fee authority, the profile's wallet); on the sponsored path the sponsor is the first two, and the profile always signs. Nothing the sponsor sees lets it take a badge.

**Issuer flow (planned).** The foundation runs the first issuer: Didit's face check (passive liveness and deduplication; Didit holds the faces), then the person's identity commitment into a list, inserted in batches. Nothing server-side holds a person next to a profile.

## What Soil needs from the foundation

- The keys recipe as a library; the host fork; the fee payer's config pattern; the record shapes and validator; the registry client (build a proof on the device, send it through the foundation's fee payer); the escrow client; the names endpoint.
- Soil adds only its page, its host instance, its seed-file store, its fee payer for everything else, the ramp and pool it offers, and the door (MCP).
...
- Soil waits a random interval, minutes to hours, between a person joining the list and their first registration, and the issuer inserts identities in batches, so a badge cannot be matched to a face check by timing.
- A write from Soil needs only the seed unlocked for the session, never a passkey gesture per write.
- When a seller writes an offer, Soil starts its terms from the market file's suggested values; the seller sets them.
...
1. Devnet deploy of both programs. First a real treasury key replaces the registry's placeholder, which anyone with this repo can sign for.
2. The issuer flow.
3. The fee payer's config.
4. The carrier's config.
5. The index, with the badge, the pay link and the read skill.
6. Names.
7. The `markets` repo: the market template and its validator, category files, the directory, the first market files.

Meeting points, in order: the issuer flow (face check to registry entry); the carrier and index with badge and link; escrow with auto-release; names; the door (in Soil).
...
- **Money.** Whether one stablecoin holds for EU users. Which ramps accept a prefilled link with no partner account. Whether Crossmint or Onramper accepts a sole proprietor for production, and whether Crossmint's bank rails cover the first sellers' countries. Which wallet holds a pool's compliance attestation: on the central wallet it adds to what the ramp already knows; on a profile wallet it would put a person next to a profile. Whether an app offering a pool needs its own lawyer pass.
- **Registry.** Confirm the wallet in the proof's message and the index rule on the declared wallet. Whether the Semaphore team publishes a transcript for the later artifacts. The per-market completeness circuit and its ceremony (the ceremony needs real independent contributors, so it waits for a public), and the proof that a sorted list matches the program's code tree, not built or costed. A later circuit proving the profile's key and the identity secret come from one seed, which closes badge selling entirely. When a second list opens. Whether the registry also emits an attestation other Solana apps can read (parked). The treasury can accept a token it mints itself, a voucher by another name; only its own discipline, a multisig and a public policy stand against that. Whether a mainnet build should refuse to compile with the placeholder treasury. How long Soil's random wait before a first registration is, sized from real joining rates.
- **Escrow** (each a program change, possible only before deploy). SOL sent to an escrow's address goes to the rent payer, not to whoever sent it. Tokens of another mint sent to an escrow's address are not recovered. An escrow funded after its own last deadline can be sent back at once, before the seller could accept. The endings the buyer does not sign pay the buyer at any token account it owns: tighten them to the refund address, or reword `CLAUDE.md`'s "refund address fixed at creation". A frozen refund address blocks the two returns anyone may send until it is unfrozen. How a sponsor caps the deadlines of the escrows whose rent it pays, since a far deadline is a far wait. Whether the wallets people use accept a program-derived address as a Solana Pay recipient (not tried on a phone).
- **Keys and host.** Whether a moved passkey keeps its PRF secret (the design assumes not). Whether a product also writes a seed file under the first passkey. What handle a new profile carries at genesis, before any foundation name. Whether the host refuses an imported export whose root does not verify; whether it validates Forest records itself; how a move tells the old host and the carrier; whether an owner can deactivate or delete a folder without a session.
- **Names.** When chosen names (v2) open and how their auction runs: its format, whether a name is held for good or renewed, and where the money goes. Whether a badged profile's active handle moves from the host's to the foundation's by default.
...
- **Registry:** Semaphore's contracts deployed unchanged; the registry as a service for other apps; free registrations inside the program (ten free tokens, free-slot codes, vouchers or blind tokens); numbered per-human codes; splitting the list on day one; depth 20; custom code inside Kora.
- **Identity and keys:** issuer-assisted recovery in v0; the face check as a product-side service; wallet infrastructure providers (Turnkey, Privy, Crossmint wallets) for keys.
...
- **Names:** a per-human handle market; one handle per human.
