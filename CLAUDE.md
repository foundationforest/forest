...
- Escrow v1: classic SPL Token mints only. Parties are keys. Each escrow has its own deposit address; refund address fixed at creation.
...
- The passkey belongs to forest.foundation; products are listed in the related-origins file. Names are `handle.forest.foundation`, badged profiles only, one per human.
