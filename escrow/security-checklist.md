...
| Testing | LiteSVM (49 tests), a local validator (the client's `test:validator`), a Trident fuzzer (twelve invariants) |
...
5. **Anyone may send `mark_funded`, `timer_release`, `recover_late` and `sweep_rent`.** Each moves
   money or lamports only to destinations fixed at creation (the side the timer names, the buyer's
   standard account, the recorded rent payer), and needs no signature.
...
- **1.1 Signer checks. Applied.** Every authority is an Anchor `Signer`: the creator and payer
  (`create`), the buyer (`release_to_seller`, `split`), the seller (`release_to_buyer`, `split`),
  ...
  `the_wrong_signer_is_refused_for_every_instruction` and, in the fuzzer, I5 and I7.
...
- **1.3 Account data matching. Applied.** `has_one = vault` and `has_one = rent_payer` on every
  way out; `has_one = buyer` and `has_one = mint` in `recover_late`; the seller's account by
  `token::mint` and `token::authority`; the buyer's by `address = escrow.refund_address()`; the
  timer's one account by side, in the handler. Tested by the `substitution_…` tests and
  `a_payout_to_the_buyer_lands_only_at_its_standard_account`.
...
- **2.2 PDA sharing. Applied.** One escrow per `["escrow", buyer, id]`; its deposit account is its
  own associated token account. No shared vault.
- **2.3 Seed collisions. Applied.** The seeds are a fixed tag and two fixed-length parts (32 and 8
  bytes), so no two (buyer, id) pairs concatenate alike. Tested by `pda_the_two_programs_cannot_share_an_address`.
...
- **3.1 Checked math. Applied.** A split is computed in 128 bits (`share`: a 64-bit balance times at
  ...
  (`TimeOverflow`), `Closed`'s rent sum by `checked_add`. `sweep_rent` uses `saturating_sub`, where a
  floor of zero is the rule (nothing to sweep). `overflow-checks = true` in the release profile.
...
- **3.4 Lamport balance. Applied.** Rent goes only to the recorded rent payer (the deposit
  account's at every ending, both rents at `close_unfunded`, the excess at `sweep_rent`), and a
  re-created deposit account's to the buyer. Fuzzer I3.

### 4. Duplicate mutable accounts

**Applied, with one documented case.** Anchor 1.2 refuses duplicate mutable `Account` fields. The
unchecked ones are pinned elsewhere: the buyer's by address, the rent payer by `has_one`, the
timer's by side. One case remains: a buyer who hands its standard account to the seller makes it
also an account the seller holds, so `split` or `arbitrate` can name it twice and both shares land
there. The program writes no data to token accounts, so nothing is corrupted; the buyer arranged
it. Refusing it would let a buyer block the arbiter, so it is allowed.
...
- **6.3 Closing. Applied.** `close = rent_payer` for the escrow account in `close_unfunded`
...
- **8.1 Atomicity. Applied.** One-tap Pay is `create`, a transfer and `release_to_seller` in one
  transaction; two ways out in one transaction revert together (`an_escrow_ends_once`).
- **8.2 Compute. Applied.** No loop over input beyond the two payouts. The heaviest single
  instruction measured is 57,220 units, the one tap 84,330 (`README.md`).
...
escrow's address waits for the same buyer to reopen the id, which adopts it. Nothing expires, so
nothing needs closing by a stranger: a never-funded escrow can be closed by either party or its rent
payer at any time; a funded one is the parties' money and ends only by their ways out.
...
buyer and seller different, creator a party, mint classic and not wrapped SOL. No strings. Any
...
- **29.1 Front-runnable initialization. Applies, as a finding.** `create` is permissionless within
  the parties: the creator must be the buyer or the seller, but whoever opens it names the seller.
  So anyone can open the address a buyer is about to use, `["escrow", buyer, id]`, as an invoice
  naming itself. No money moves: one tap fails whole, and the squatted escrow never held anything,
  so the buyer can close it and reopen the id. An app that pays apart from its own `create` checks
  `whatDiffers` first. Pinned by `finding_anyone_can_open_the_address_a_buyer_is_about_to_use`.
...
- **31.4 Recipients that can receive and move tokens. Applied:** the buyer is paid at its standard
  account, the seller at a token account it holds; each holder can move what it gets.
- **31.5 Token-2022 mint space. Does not apply.**
- **31.6 Signer-as-new-account. Does not apply:** PDAs only. The PDA analogue is 29.1.
...
- **§1.1 Account types.** Every unchecked account has a `/// CHECK:` comment:
  - `rent_payer` (every way out, `close_unfunded`, `sweep_rent`): the recorded key, by `has_one`; it
  ...
  - `to` (`timer_release`): checked in the handler against the side the timer names
    (`check_sellers_account`: owner, unpack, holder, mint; or the buyer's address).
...
- **§2.5 `close`.** Used in `close_unfunded`, to the recorded rent payer.
...
three deals built by the client and sent through `solana-test-validator`). Trident's runtime checks
...
1. **A frozen token account.** A classic mint's freeze authority (USDC has one) can freeze:
   ...
   - a seller's account, after which the seller names another it holds.
...
3. **A front-run address** (29.1). Anyone can open a buyer's next address first. No money moves,
   and the buyer can take the address back.
4. **A part payment can be closed under the buyer** by the seller or the rent payer, at any time.
   The part comes back; a second part sent to the closed address waits for the buyer to reopen the
   id (`finding_a_part_payment_can_be_closed_under_the_buyer_by_the_seller_or_the_rent_payer`).
5. **An overpayment follows the balance** (High-risk decision 4).
6. **Open: where the seller is paid.** Any token account the seller holds, and for
   `timer_release` the sender chooses which. The money is the seller's either way, but may land in
   an account its wallet does not show
   (`finding_anyone_may_send_the_sellers_timer_to_any_account_the_seller_holds`). Whether the seller
   too should be paid only at its standard account is open.
...
8. **SOL sent to an escrow's address** goes to the rent payer by `sweep_rent`, not to whoever sent
   it; the program cannot tell it from rent.
